...
/* Komponen referensi, disalin apa adanya — tidak dirender di halaman ini.

   Yang dipakai portofolio ini animasinya, bukan kisinya: keyframes `slide-in`
   + `animation-timeline: view()` + tangga `transform-origin` per kolom. Semua
   itu dipasang ulang di .pcard-fx (app/portfolio.css) supaya menempel pada
   kartu proyek yang sudah ada, yang dibangun runtime dari daftar PROJECTS,
   bukan dari komponen React.

   Dua hal membuat MasonryGrid tidak bisa dipakai langsung di sini:
   1. Kisi proyeknya `auto-fill minmax(300px, 1fr)` — jumlah kolomnya berubah
      mengikuti lebar jendela, sementara tangga transform-origin di bawah
      dipaku ke nth-of-type 2/4/6. Di sini jumlah kolomnya dibaca runtime dan
      dioper sebagai --origin per kartu (tagCardFx di portfolio-runtime.js).
   2. Kelas rupanya (bg-card, text-muted-foreground, text-primary, border)
      token shadcn yang tidak ada di proyek ini — paletnya sendiri
      (--ink/--canvas/--soft) di portfolio.css.

   Ditinggalkan sebagai rujukan: kalau angka di .pcard-fx diubah, ini asalnya. */
