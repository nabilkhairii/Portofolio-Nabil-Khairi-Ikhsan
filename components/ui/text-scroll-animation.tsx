...
/* Skiper31 (skiper-ui) — komponen referensi, disalin apa adanya.

   Satu baris yang berbeda dari aslinya: import motion dari "motion/react",
   bukan "framer-motion". Keduanya pustaka yang SAMA — framer-motion berganti
   nama jadi motion — dan proyek ini sudah memakai motion v13 di origin-button
   dan scroll-choreography. Memasang framer-motion di sampingnya berarti dua
   salinan pustaka animasi yang sama di satu bundel, dengan konteks
   MotionConfig masing-masing.

   Berkas ini TIDAK dirender di halaman: Skiper31 sebuah demo satu layar penuh
   (latar putih, teks "see more from", ikon dari CDN simple-icons). Yang dipakai
   portofolio ini matematika CharacterV1-nya — huruf berangkat terpencar
   sejauh (indeks - tengah) x 50 lalu menyatu — dan itu dipasang lewat
   .split-fly di app/portfolio.css, bukan lewat React. Alasannya dua, keduanya
   terbukti di proyek ini:

   1. useScroll rusak oleh `zoom` di <body> (lihat catatan --zoom di
      portfolio.css): progresnya mentok, tanpa error. Sudah kena sekali di
      pita koreografi.
   2. Kesembilan judulnya diterjemahkan dengan menimpa innerHTML
      (applyLang di portfolio-runtime.js). Pulau React di sana akan disapu
      tiap kali bahasa ditukar, dan React tidak akan tahu.

   Ditinggalkan di sini sebagai rujukan efeknya — kalau angkanya diubah di
   .split-fly, ini sumber aslinya. */
